...
Keyboard.prototype.toggleOne = function(key,duration, delay) {
    const DURATION_MIN = 100; // minimum 100 ms. Any lower and it doesn't seem to register
    const DURATION_MAX = 2500; // 2.5 seconds
	const DELAY_MIN = 100;
	const DELAY_MAX = 100;
...
Keyboard.prototype.dequeue = function() {
    ...
    const key = this.commands[0].key;
    const duration = this.commands[0].duration;
	const delay = this.commands[0].delay;
